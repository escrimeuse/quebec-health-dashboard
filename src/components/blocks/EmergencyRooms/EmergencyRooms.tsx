...
export function EmergencyRooms({region}: {region: Region}) {
    ...
    const {data, isLoading} = useQuery({queryKey: ['emergency-room-data'], queryFn: async () => {
            ...
            <CardHeader>
                ...
                <CardAction className="gap-2 flex flex-row content-center items-center">
                    ...
                    {SORT_BY.map((sortMethod) => {
                        return <ToggleGroupItem key={sortMethod.value} value={sortMethod.value} size="sm" onClick={() => setSort(sortMethod.value)}>{sortMethod.label}</ToggleGroupItem>
                    })}
                </ToggleGroup>
                </CardAction>
            </CardHeader>
                ...
                {filteredData?.map((data) => {
                    ...
                    return (
                        <Item key={data.No_permis_installation} variant="muted">
                                ...
                                <ChartContainer config={{patientsWaitingFirstVisit: {label: 'Patients Waiting To Be Seen'}, patientsSeen: {label: "Patients Already Seen"}}}>
                                    <PieChart>
                                        <ChartTooltip
                                            ...
                                            />
                                        <Pie data={data.totals} dataKey="count" nameKey="category" innerRadius={70} strokeWidth={5}>
                                            <Label
                                                content={({ viewBox }) => {
                                ...
                                <ChartContainer config={{occupiedStretchers: {label: 'Patients in stretchers less than 24 hours'}, occupied24Hours: {label: "Patients in stretchers more than 24 hours"}, occupied48Hours: {label: "Patients in stretchers more than 48 hours"}}}>
                                    <PieChart>
                                        <ChartTooltip
                                            cursor={false}
                                            content={<ChartTooltipContent />}
                                            />
                                        <Pie data={data.stretchers} dataKey="count" nameKey="category" innerRadius={70} strokeWidth={5}>
                                            <Label
                                                content={({ viewBox }) => {
                                                if (viewBox && "cx" in viewBox && "cy" in viewBox) {
                                                    return (
                                                    ...
                                                    >
                                                        ...
                                                        className="fill-foreground text-3xl font-bold"
                                                        >
                                                        {data.Nombre_de_civieres_occupees} / {data.Nombre_de_civieres_fonctionnelles}
                                                        </tspan>
                                                        <tspan
                                                        x={viewBox.cx}
                                                        y={(viewBox.cy || 0) + 24}
                                ...
                                <div className="gap-8 flex flex-col content-center justify-center content-stretch">
                                    ...
                                    <div className="gap-1 flex items-center flex-col">
                                        <span className="text-5xl font-bold">{stretcherWaitHours}h {stretcherWaitMinutes}m</span>
                                        <p className="text-lg">Average wait time on stretcher</p>
                                    </div>

                                    <div className="gap-1 flex items-center flex-col">
                                        <span className="text-5xl font-bold">{ambulatoryWaitHours}h {ambulatoryWaitMinutes}m</span>
                                        <p className="text-lg">Average wait time in waiting room</p>
                                    </div>
