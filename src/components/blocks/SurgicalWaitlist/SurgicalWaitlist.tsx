...
export function SurgicalWaitlist({region}: {region: Region}) {
  ...
  const {data, isLoading} = useQuery({queryKey: ['surgical-waitlist-data', delay, region], queryFn, });
  ...
  return (
    <>
      <Card>
            ...
            <CardAction>
                <ToggleGroup type="single" value={delay} variant="outline">
                    {Object.keys(WAIT_LABELS).map((key) => {
                        return <ToggleGroupItem key={key} value={key} size="sm" onClick={() => setDelay(key as Delay)}>{WAIT_LABELS[key as keyof typeof WAIT_LABELS]}</ToggleGroupItem>
                    })}
                </ToggleGroup>
        ...
        <CardContent>
          {isLoading || data === undefined ? <p>Loading</p> : <SurgicalWaitlistChart data={data} />}
        </CardContent>
      </Card>
    </>
  )
...
function SurgicalWaitlistChart({ data }: { data: SurgicalWaitlist[]}) {
  ...
  return (<ChartContainer config={CHART_CONFIG}>
      <LineChart accessibilityLayer data={transformed}>
        ...
        <XAxis xAxisId={0} dataKey="xAxis" allowDuplicatedCategory={true} tick={CustomizedAxisTick} interval={0} tickLine={true} axisLine={true}></XAxis>
